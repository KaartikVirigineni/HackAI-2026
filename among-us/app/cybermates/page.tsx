...
export default function PhishingPier() {
  ...
  const { playHover, playWin, playLose, playWalk, stopWalk, startBgMusic } = useCyberAudio();

  useEffect(() => {
    // Attempt to start music on first mount or interaction
  ...
  useEffect(() => {
    ...
    newSocket.on('game_over', (data: { winner: string; reason: string; eloGained?: number; bonusStr?: string }) => {
      setGameOver(data);
      if (
        (data.winner === 'blue' && players[newSocket.id]?.team === 'blue') ||
        (data.winner === 'red' && players[newSocket.id]?.team === 'red')
      ) {
        playWin();
      } else {
        playLose();
