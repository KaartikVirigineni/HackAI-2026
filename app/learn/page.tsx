...
export default function LearnPage() {
  ...
  useEffect(() => {
    if (socket && !lobbyActive) {
      ...
      const onLessonSync = (data: { lesson: Lesson, answers: Record<string, number>, revealed: Record<string, boolean> }) => {
        setLesson(data.lesson);
        setUserAnswers(data.answers || {});
        setRevealedExplanations(data.revealed || {});
      };

      socket.on('lesson_sync', onLessonSync);
      return () => { socket.off('lesson_sync', onLessonSync); };
    }
  ...
  const fetchLesson = async (feedback?: 'up' | 'down') => {
    ...
    try {
      ...
      setLesson(data);
      if (socket && !lobbyActive) {
        socket.emit('broadcast_lesson', { roomId: roomName, lesson: data, answers: {}, revealed: {} });
      }
    } catch (err) {
  ...
  const handleSelectAnswer = (questionId: string, optionIndex: number) => {
    ...
    const newRevealed = { ...revealedExplanations, [questionId]: true };
    
    setUserAnswers(newAnswers);
    setRevealedExplanations(newRevealed);
    
    if (socket && !lobbyActive && lesson) {
      socket.emit('broadcast_lesson', { roomId: roomName, lesson, answers: newAnswers, revealed: newRevealed });
    }
  };
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 py-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-8 flex flex-col relative">
            ...
            {/* Render lesson content when we are not in the lobby */}
            {!lobbyActive && (
...
function InteractiveLearnSession({
  lesson, userAnswers, revealedExplanations, handleSelectAnswer, loading, error, fetchLesson, playHover
}: {
  lesson: Lesson | null,
  userAnswers: Record<string, number>,
  revealedExplanations: Record<string, boolean>,
  handleSelectAnswer: (questionId: string, optionIndex: number) => void,
  loading: boolean,
  error: string | null,
      ...
      <div className="relative z-10">
        {loading ? (
          <div className="flex flex-col items-center justify-center py-20 gap-6">
            ...
            <p className="text-cyber-green animate-pulse font-orbitron tracking-widest uppercase text-sm">Intercepting Cyber Intel...</p>
          </div>
        ) : error ? (
          ...
          <article className="space-y-10 animate-fade-in-up">
            <header className="mb-8 border-b border-cyber-blue/30 pb-4">
              <div className="flex justify-between items-center mb-2">
                ...
                <span className={`font-mono text-xs font-bold uppercase ${lesson.difficulty === 'Advanced' ? 'text-red-500' : lesson.difficulty === 'Intermediate' ? 'text-yellow-500' : 'text-green-500'}`}>
                  [{lesson.difficulty}]
                </span>
              </div>
            ...
            <div className="bg-[#050A1F]/60 p-8 rounded-xl border border-cyber-blue/20 shadow-inner max-h-[600px] overflow-y-auto cyber-scrollbar pr-6">
               <div className="prose prose-invert max-w-none">
                  <ReactMarkdown 
                    remarkPlugins={[remarkGfm]}
                    components={{
                      p: ({ node: _node, ...props }) => <p className="mb-4 last:mb-0 leading-relaxed text-gray-300" {...props} />,
                      a: ({ node: _node, ...props }) => <a className="text-cyber-green underline hover:text-white transition-colors" target="_blank" rel="noopener noreferrer" {...props} />,
                      ul: ({ node: _node, ...props }) => <ul className="list-disc pl-6 mb-4 space-y-2 text-gray-300" {...props} />,
                      ol: ({ node: _node, ...props }) => <ol className="list-decimal pl-6 mb-4 space-y-2 text-gray-300" {...props} />,
                      li: ({ node: _node, ...props }) => <li className="text-gray-300" {...props} />,
                      code: ({ className, children, inline, ...props }: { node?: unknown, inline?: boolean, className?: string, children?: React.ReactNode }) => {
                        const match = /language-(\w+)/.exec(className || '');
                        return !inline && match ? (
                          <div className="relative my-6 group">
                            <div className="absolute -inset-1 bg-cyber-green/20 rounded-lg blur opacity-25 group-hover:opacity-50 transition duration-1000"></div>
                            <pre className="relative bg-cyber-dark/80 p-5 rounded-lg border border-cyber-green/30 font-mono text-sm overflow-x-auto cyber-scrollbar">
                              <code className={className} {...props}>{children}</code>
                            </pre>
                          </div>
                        ) : (
                          <code className="bg-cyber-green/10 text-cyber-green px-1.5 py-0.5 rounded font-mono text-xs border border-cyber-green/20" {...props}>
                            {children}
                          </code>
                        );
                      },
                      blockquote: ({ node: _node, ...props }) => (
                        <blockquote className="border-l-4 border-cyber-green/50 bg-cyber-green/5 p-4 rounded-r-lg my-6 italic text-gray-400" {...props} />
                      ),
                      table: ({ node: _node, ...props }) => (
                        <div className="overflow-x-auto my-8 rounded-lg border border-cyber-green/20">
                          <table className="w-full text-left border-collapse" {...props} />
                        </div>
                      ),
                      thead: ({ node: _node, ...props }) => <thead className="bg-cyber-green/10 text-cyber-green font-orbitron text-xs uppercase tracking-wider" {...props} />,
                      th: ({ node: _node, ...props }) => <th className="p-4 border-b border-cyber-green/20" {...props} />,
                      td: ({ node, ...props }: { node?: unknown }) => <td className="p-4 border-b border-cyber-green/10 font-normal" {...props} />,
                      h1: ({ node: _node, ...props }) => <h1 className="text-3xl font-orbitron font-bold text-white mb-8 mt-12 text-glow-green" {...props} />,
                      h2: ({ node: _node, ...props }) => <h2 className="text-2xl font-orbitron font-bold text-cyber-blue mb-6 mt-10" {...props} />,
                      h3: ({ node: _node, ...props }) => <h3 className="text-xl font-orbitron font-bold text-cyber-green mb-4 mt-8" {...props} />,
                    }}
                  >
                    {lesson.content}
                  </ReactMarkdown>
            ...
            {/* MCQs Section */}
            <div className="mt-16 space-y-12 border-t border-cyber-green/20 pt-12">
              <h2 className="text-2xl font-orbitron font-bold text-cyber-green uppercase tracking-[0.2em] flex items-center gap-3">
                <span className="animate-pulse text-3xl">🧩</span>
                Knowledge Validation
              </h2>
              
              {lesson.questions.map((q, qIndex) => (
                <div key={q.id} className="bg-cyber-dark/40 border border-cyber-blue/20 rounded-xl p-8 space-y-6">
                  <h3 className="text-xl text-white font-medium flex gap-4">
                    <span className="text-cyber-blue font-orbitron">Q{qIndex + 1}.</span>
                    {q.question}
                  </h3>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {q.options.map((opt, optIndex) => {
                      ...
                      let btnStyle = "bg-cyber-darker border-cyber-blue/20 text-gray-400 hover:border-cyber-blue/50 hover:bg-cyber-blue/10";
                      if (hasAnswered) {
                         if (isCorrect) {
                           btnStyle = "bg-cyber-green/20 border-cyber-green text-cyber-green shadow-[0_0_15px_rgba(0,255,157,0.2)]";
                         } else if (isSelected) {
                            btnStyle = "bg-red-500/20 border-red-500 text-red-500";
                         } else {
                            btnStyle = "bg-cyber-darker/40 border-gray-800 text-gray-600 opacity-50";
                         }
                      }
                      ...
                      return (
                        <button
                          ...
                          className={`p-4 rounded-lg border text-left transition-all duration-300 flex items-center gap-4 ${btnStyle}`}
                        >
                          <span className="font-orbitron font-bold text-xs opacity-50">{String.fromCharCode(65 + optIndex)}</span>
                          {opt}
                        </button>
                      );
                    })}
                  ...
                  {revealedExplanations[q.id] && (
                    <div className={`mt-6 p-4 rounded-lg border leading-relaxed animate-fade-in ${userAnswers[q.id] === q.correctAnswerIndex ? 'bg-cyber-green/5 border-cyber-green/30 text-cyber-green/90' : 'bg-red-500/5 border-red-500/30 text-red-200'}`}>
                      <p className="text-sm">
                        <strong className="uppercase font-orbitron text-xs block mb-1">System Feedback:</strong>
                        {q.explanation}
                      </p>
                    </div>
                  )}
          ...
          <div className="mt-16 pt-12 border-t border-cyber-blue/20 relative animate-fade-in-up">
            ...
            <div className="text-center space-y-6 bg-cyber-dark/80 backdrop-blur-md p-8 sm:p-12 rounded-2xl border border-cyber-blue/40 shadow-[0_0_30px_rgba(0,243,255,0.05)]">
              ...
              <p className="text-cyber-green/80 text-lg max-w-2xl mx-auto">
                You&apos;ve completed the intelligence deep-dive. Your tactical knowledge is now ready for live fire simulation.
              </p>
              <div className="pt-4">
                <Link
                  href="/trivia"
